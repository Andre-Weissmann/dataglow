...
reusable capabilities that agents and humans should know about. Moved out of
`AGENTS.md` so the agent instructions stay short and this log can grow without
context-rot on every session start.

When you ship a new foundation, add a one-paragraph entry here in the same PR.
Do not paste the full log back into `AGENTS.md`.
