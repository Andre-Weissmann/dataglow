...
| The Bench: dataset lineage tracking, tab wiring + Trust Strip integration (Batch 3 of 3, complete) | `js/app-shell/bench-shell.js` (engine, Batch 1), `js/app-shell/main.js` (`logBenchStep`/`renderBenchStoryStrip`/`resetBenchLineage`, Batch 2), `js/trust/trust-strip.js` + `js/trust/proof-drawer.js` (`benchLineage` param, Batch 3) | LIVE (`theBench`, promoted) | MED | Pure lineage algebra (`createLineage`/`addStep`) + a thin story-strip renderer (`renderStoryStrip`) from Batch 1, wired live into the SQL/Python/R tabs in Batch 2 (an `upload`/`sql`/`python`/`r`/`clean` step logs on every real dataset load, successful run, or applied cleaning fix respectively). Batch 3 connects that SAME session lineage into the ALREADY-LIVE Trust Strip + Proof Drawer (OneCanvas Phase 1, `trustStripProofDrawer`, shipped ON) on the Validate tab — deliberately reusing that existing trust surface instead of building a second, competing one. The Trust Strip's Lineage field now names the session's Bench-story step count alongside the dataset's own provenance-chain count, and clicking it opens the Proof Drawer with the real Bench steps as their own distinctly-labelled block next to the usual attestation. Proven end-to-end by `test/e2e-bench-trust-strip.test.mjs`: run SQL, switch to Validate, the field updates live with no manual re-render, click it, the real step names appear in the drawer. `test/e2e-bench-story-strip.test.mjs` (Batch 2) continues to prove the cross-tab story-strip parity. Both e2e tests confirm the flag-off default stays byte-for-byte unchanged. A real shared-grid rendering engine (replacing three separate SQL/Python/R result panes with one continuous grid) remains a separate, larger, not-yet-scoped lift beyond this 3-batch build. |
...
| Proof room | `js/provenance/proof-room.js`<br>`js/app-shell/tabs/proof-room-tab.js` | LIVE | MOAT | Composites readiness gate + ledger + seal into one proof. |
...
| Trust Passport (Batch 1: engine) | `js/provenance/trust-passport.js`<br>`test/trust-passport.test.mjs` | SHIPPED | HIGH | Pure composition engine answering "can I trust this, right now, in one look" without visiting four separate screens. `buildTrustPassport()` composes the EXISTING outputs of the readiness gate, AI Touch Ledger, Ownership Ledger and Agent Action Firewall into one object: readiness score, touch/ownership summary, and a fixed five-capability agent-permission table (read-aggregate: allow, read-row-level: deny, propose-repair: gate, auto-apply-repair: deny, export-outside-device: gate). Invents no new ledger, crypto, or storage; never throws on missing/malformed input. Wired into a real UI panel by Batch 1.5 (below); flag `trustPassport` enabled 2026-09-24 after live verification. |
| Trust Passport Export & Independent Verify (Batch 2) | `js/provenance/trust-passport-export.js`<br>`test/trust-passport-export.test.mjs` | SHIPPED | HIGH | Makes a Trust Passport portable and independently verifiable. Reuses the exact Merkle/SHA-256 primitives already shipped in `verifiable-check-seal.js` (`sealCheckResult`/`verifySeal`) rather than inventing new cryptography. `sealTrustPassport()` seals every passport field (readiness, touch/ownership summary, full permission table) into a Merkle-tree commitment; `verifyTrustPassportExport()` recomputes hashes offline, with no DataGlow install and no shared secret, and detects any edit to any field after export. `exportTrustPassport()` renders json/markdown/text. Tamper-evidence only, not a signature or certification; disclaimer shipped on every export. Its seal/verify/export controls render inside the Batch 1.5 panel only when `trustPassportExport` is also on. Flag `trustPassportExport` enabled 2026-09-24 after live verification (real seal + download confirmed). |
| Trust Passport Panel (Batch 1.5: UI mount) | `js/provenance/trust-passport-panel.js`<br>`test/trust-passport-panel.test.mjs` | SHIPPED | HIGH | Mounts Batch 1 + Batch 2 into a real Trust Passport tab: `buildTrustPassportPanelPlan()` is a pure, dataset-loaded readiness check; `renderTrustPassportPanel()` is the DOM presenter (summary card, permissions table, optional seal/export/verify card). Wired into `js/app-shell/main.js` (`TAB_META`, `getVisibleTabIds()`, `switchTab` dispatch, `renderTrustPassportTab()`), `js/app-shell/state.js` (`tabOrder`), `js/app-shell/command-deck-nav.js` (Trust stage), and a static panel section in root `index.html`. Gated behind the same `trustPassport` flag as the engine (enabled 2026-09-24); export controls inside the panel additionally require `trustPassportExport` (also enabled 2026-09-24). |
...
| Guarded Copilot | `js/agents/guarded-copilot.js`<br>`js/app-shell/tabs/guarded-copilot-tab.js` | LIVE | HIGH | Read-only chat assistant. Cites proof chain. Cannot modify data by construction. |
...
| Excel workbook reader | `js/shared/excel-import.js` | LIVE | CORE | Turns an .xlsx or .xls sheet into DataGlow's positional-array rows using the vendored SheetJS build at `assets/xlsx/`. Detects and reports header-not-on-row-1, blank leading rows, trailing empty columns, merged ranges, Excel serial dates and numbers stored as text instead of silently repairing them. |
...
### Runtime supply chain, the derived CSP and the desktop sidecar

DataGlow's privacy claim is about egress and it is true. It is also not the whole story: four heavy runtimes are fetched from public CDNs on first use, and each one is code that will execute with full access to a page that has just read someone's payroll file.

**Runtime pin policy** (`js/ai/model-supply-chain.js`, `window.DataGlowModelSupplyChain`, flag `modelSupplyChainPin`). Subresource Integrity is the right answer for a script tag and reaches neither of the two things that matter here: WebLLM is loaded by a dynamic `import()`, which takes no integrity attribute, and the model weights are over a gigabyte fetched in shards by the runtime itself. So the honest control is a pinned immutable version URL plus a short allowlist of the exact origins those URLs live on, written down once instead of spelled slightly differently in four loaders. `pinned` is derived from the shape of the URL rather than asserted by hand, so a loader that quietly moves to a floating reference becomes a finding instead of a comment nobody updated: one runtime resolves through a `latest` path today, it is listed with `pinned: false`, and the policy reports it, because a supply-chain module that lists only the things already fine is decoration. `buildModelFetchPolicy` decides per runtime, with Air-Gap outranking everything except a runtime the browser has already cached, since using a cached runtime fetches nothing and refusing it would cost a capability to enforce a rule it does not break. Model ids are an allowlist and never a text box, with exact matching and no prefix rule, because `Qwen2.5-1.5B-Instruct-evil` has the right prefix. `verifyPinnedRuntimeMeta` lets a loader hand over the URL string it actually holds, so drift between the pin and the loader is found by a check rather than by a user. `WEIGHT_DELIVERY_ORIGINS` is deliberately outside the pin list: a weight request to huggingface.co is answered by that project's large-file CDN on a different host, so a policy naming only the origin in the URL blocks the download at the redirect and looks to a user exactly like the model failing for no reason. It is the delivery host for one runtime, not a fifth runtime, and keeping it separate stops anything counting it as another piece of code being fetched.

**Derived Content Security Policy** (`js/security/csp-policy.js`, `window.DataGlowCspPolicy`). A CSP is the control that still works after something else has gone wrong, and for a product whose promise is that nothing leaves the machine, a `connect-src` naming four origins and nothing else is close to that promise in a form the browser enforces. It is computed from `pinnedOrigins()` rather than typed, and the reason is not tidiness: the hand-written desktop policy had already drifted, permitting two CDNs that no loader in the product uses while blocking the three that the language model runtime, the R runtime and the weights actually come from. On desktop, today, it silently blocked three advertised features. `checkPolicyCoversRuntimes` is asserted against the real `src-tauri/tauri.conf.json`, so that cannot recur quietly. The policy is not as tight as a policy can be, and the three gaps are recorded in `CSP_RESIDUALS` with what each costs: `'unsafe-inline'` for scripts, because the shipped artifact is one HTML file whose application code is inline and removing it removes the product; `'unsafe-eval'`, because DuckDB compiled to WebAssembly instantiates through a path it covers and the narrower `wasm-unsafe-eval` is not honoured everywhere this has to run; and `blob:` in script-src and worker-src, because DuckDB and Pyodide both start workers from blob URLs they build at runtime. Writing a policy that omits these produces a document that scores better and does not work. Under Air-Gap the runtime, font and delivery origins are dropped entirely, which makes the browser enforce what the mode already promises instead of relying on our own code to be careful.

**Chip readiness** (`js/narrative/ondevice-llm.js`, `js/ai/data-glow-local-ai-canvas.js`). The status chip shipped with nothing to read. Its inlined `window.OnDeviceLLM` footer published four names the module has never exported, so each resolved to its fallback and the chip could not tell a loaded model from an absent one. Underneath that, `isModelLoaded()` returned `enginePromise != null`, which is true from the instant someone clicks through the entire multi-minute weight download, so a chip built on it would have read ready while nothing could generate a word. Loaded and loading are now two separate observed facts, both cleared when a load fails and both cleared by `clearModelCache`. The four legacy names stay published, because something may read them and a sudden `undefined` is worse than a `null`.

**Desktop llama.cpp path** (`js/ai/desktop-local-llm.js`, `window.DataGlowDesktopLocalLlm`, flag `desktopLlamaCpp`). A desktop build can run a far larger model than a browser tab, through a llama.cpp server shipped beside the app as a Tauri sidecar. That binary is not in this build, and pretending otherwise is the exact failure this module exists to prevent, so the four states are literal: `unavailable_web` in a browser, `sidecar_missing` on a desktop build with no binary, `ready` only when one is actually present, `error` when it is present and failed. Running in a browser outranks every other signal, because a browser cannot have a sidecar under any circumstances and a status that says otherwise is broken rather than optimistic. `tauri.bundle.externalBin` stays an empty array on purpose: Tauri resolves each entry to a per-target-triple file at bundle time, so a named entry with no binary behind it fails the desktop build rather than shipping a promise. `describeSidecarScaffold` writes down the config path and the five steps, so turning this on later is an edit and not an investigation.

### The RECEIPT spine

**Start-here rail** (`js/spine/receipt-spine.js`, `window.DataGlowReceiptSpine`, flag `receiptSpine`). The product has all the pieces of a coherent path and no visible path, so the five steps are named across the bottom of the page: Drop, Ask, Prove, Ship, Compound. The rail deletes nothing. Every tab, panel and button stays exactly where it was, because removing surfaces is a redesign and this is a signpost. Each step is marked done, current, todo or skipped from what is actually on screen rather than from a stored counter, so a refresh or an arrival mid-session does not reset a progress bar that was never measuring progress. Shipping without proving is reported as `skipped` rather than counted as done, because that is the one sequence worth naming out loud. The canvas surface `js/spine/data-glow-receipt-spine-canvas.js` (`window.DataGlowReceiptSpineUI`) maps each step onto a real surface and re-observes on an interval; a step whose target is not mounted in this build renders as text and says so, since a button that opens nothing is worse than a sentence. Dismissing the rail is remembered and leaves a small chip to bring it back.

### Polyglot power packs

One panel, `js/polyglot/data-glow-power-packs-canvas.js` (`window.DataGlowPowerPacksUI`), holding the starter material for the four places a person writes something in this product. Every tab leads with what the runtime cannot do before it offers anything to run, because the reason people distrust in-browser runtimes is not that they are limited, it is that they discover the limit ten minutes in, from an error, after building on the assumption it was not there. Copy is the only action: the panel never runs anything and never inserts into an editor behind your back.

**SQL** (`js/polyglot/sql-power-pack.js`, `window.DataGlowSqlPowerPack`, flag `sqlPowerPack`). Ten snippets for the queries people rewrite from scratch every time, each naming the placeholders to replace, since a query run against a table called `your_table` is a wasted click. Twelve places DuckDB and Postgres part company are shown with both dialects side by side and which of the two is the odd one out, because the expensive way to learn that `QUALIFY` exists in one and not the other is at three in the morning against a warehouse.

**Spreadsheets.** The Excel tab is an entry point rather than a snippet list, because the work there is repairing a file and not writing a query. It opens the existing Excel Hell Repair panel and the saved repair methods library, and states the three things neither will do: it does not write back to the original file, it does not apply a repair without being told to, and it does not read a password-protected workbook or evaluate macros.

**Python** (`js/polyglot/python-power-pack.js`, `window.DataGlowPythonPowerPack`, flag `pythonPowerPack`). Pyodide is real CPython with real pandas, and the thing that catches people is not a missing library, it is that the dataframe crossing the bridge is capped: a truncated frame gives an answer of the right shape and the wrong value, with nothing on screen saying so. The truncation notice is therefore the first thing in the tab and states the delivered count against the real one. The default limit is a documented parameter, and a test reads `PY_BRIDGE_ROW_LIMIT` out of `js/runtimes-viz/python-runtime.js` and pins the default to it, so the fallback and the truth cannot drift apart silently.

**R** (`js/polyglot/r-power-pack.js`, `window.DataGlowRPowerPack`, flag `rPowerPack`). WebR is R and it is not CRAN. Two optional packages may or may not have installed in a given session, and the kernel is the only thing that knows because it is what tried, so it now answers a `packages()` call instead of leaving anything outside to guess. Recipes needing a package that did not install are still listed, under a heading saying they cannot run here and naming the missing package, because someone who came looking for ggplot should learn that in the panel rather than from an error. Absent any answer the pack assumes neither installed, which marks more recipes unavailable than may be true; overstating what installed is the worse error, since it hands out a call that fails.

### SQL, Excel, Python and R power-deepen; Arrow bridge; CSV quarantine; llama.cpp packaging

Bundle 13 goes one layer past the starter material in `js/polyglot/data-glow-power-packs-canvas.js` (`window.DataGlowPowerPacksUI`): deeper recipes, a real profiling engine, a real Python probe, a real Air-Gap check for R, and the pieces the CSV loader and the llama.cpp sidecar path were still missing.

**SQL deepen** (`js/polyglot/sql-deepen.js`, `window.DataGlowSqlDeepen`, flag `sqlPowerDeepen`). `summarizeToTiles` turns a DuckDB `SUMMARIZE` result into Proof Board tiles instead of leaving it as a wide dump nobody reads: a null rate over the honest threshold, a column with exactly one distinct value, a column that looks like a key. Every tile carries the SQL that produced it and a `gateBadge` of `'unknown'`, because a profiling finding is an observation and not an adjudication the Proof Board gates get to make on this module's behalf. `SQL_DEEPEN_SNIPPETS` adds eleven templates past the existing ten in `js/polyglot/sql-power-pack.js` — ASOF join, PIVOT/UNPIVOT, `QUALIFY` top-N, `GROUP BY ALL`, `SELECT * EXCLUDE`/`REPLACE`, moving average, a time-based trailing window, share-of-total, and the SUMMARIZE profile query itself — into the same SQL power pack drawer.

**Excel type guard** (`js/intelligence/excel-type-guard.js`, `window.DataGlowExcelTypeGuard`, flag `excelTypeGuard`). The gene-symbol-to-date coercion (SEPT2 to 2-Sep) that got twenty-seven genes renamed in 2020, generalized to part numbers, scientific-notation identifiers and leading zeros. `detectTypeRisks` separates `already_coerced` (the damage already happened upstream; the original text may be gone) from `at_risk` (would be converted on the next spreadsheet round trip; nothing is wrong yet), because claiming to fix the first case would be a guess dressed as a repair. `previewGuard` only ever offers holding a column as text, and `typeGuardReceiptLine` writes a record on apply or override so a decision is never silent.

**Power Query honest note** (`js/polyglot/power-query-note.js`, `window.DataGlowPowerQueryNote`, flag `powerQueryHonestNote`). One sentence, not a comparison table: Power Query M is not embedded here. `POWER_QUERY_EQUIVALENTS` names five real surfaces that do the same job (Excel Hell Repair, DuckDB SQL, Python/R, the saved repair recipe library) and the one thing this product genuinely does not have, a connector to an external database or API. The same sentence is duplicated verbatim into `js/ai/capability-ceiling.js` (`POWER_QUERY_CEILING_NOTE`) for the ceiling group, and a test pins the two strings equal so the copy cannot drift.

**Python deepen** (`js/polyglot/python-deepen.js`, `window.DataGlowPythonDeepen`, flag `pythonPowerDeepen`). Bundle 11 shipped a Polars scaffold whose availability answer was a hard-coded `false`. This replaces it with a real probe: `PROBE_CELL` asks the Pyodide session about polars, scikit-learn, statsmodels and pyarrow one line at a time, and `packageAvailability` reports `available`, `not_loaded` (with a reason), or `unknown` for a session nobody has probed yet — never a guess. `buildPythonDeepen` splits its recipes into runnable and blocked, and a blocked recipe always carries the reason and, when relevant, the Air-Gap-aware note that installing a package is a network request this build does not make for you.

**R deepen** (`js/polyglot/r-deepen.js`, `window.DataGlowRDeepen`, flag `rPowerDeepen`). Ten dplyr/ggplot2 recipes, each blocked recipe pointing at a base R alternative that always runs. The reason this exists: `js/runtimes-viz/r-runtime.js` was calling `installPackages` at WebR startup with no check against Air-Gap mode anywhere in it, so a mode whose entire promise is that nothing leaves the machine was making two package fetches on the way up. `packageInstallDecision` is the pure gate — `blockedBy: 'air_gap'` or `'offline'` — and the runtime's new `rPackageInstallAllowed()` asks it before calling `installPackages`, degrading to base R only rather than failing the session.

**Arrow bridge status** (`js/polyglot/arrow-bridge.js`, `window.DataGlowArrowBridge`, flag `arrowBridge`). The DuckDB-to-pandas handoff today is a `LIMIT 200000` query walked into plain objects, JSON-stringified, and parsed back by pandas — a full copy of the data in a format neither end wanted. `buildArrowBridgeStatus` reports `ready` / `partial` / `missing` from what is actually observed about both ends, and `describeArrowStepUp` writes down which steps toward the zero-copy Arrow path are done (probing pyarrow) and which are not (exposing the DuckDB Arrow buffer, writing it across, raising the limit). `NEVER_UNLIMITED` is asserted directly: an Arrow path removes the parse cost, not the fact that a frame has to fit in one browser tab's memory.

**CSV quarantine** (`js/dataquality/csv-quarantine.js`, `window.DataGlowCsvQuarantine`; canvas `js/dataquality/data-glow-csv-quarantine-canvas.js`, `window.DataGlowCsvQuarantineUI`). The CSV loader already asked DuckDB for the rejected-row count via `ignore_errors`/`store_rejects` and then dropped the rejects table before anyone could see which rows or why. This module reads the rejects table first: `buildQuarantine` turns DuckDB's reject rows into a model with the dropped percentage, the rows grouped by failure reason, and exactly two choices — keep the rows that parsed, or abandon the load — never a silent default. `quarantineReceiptLine` writes a record on either outcome, because a total computed after silently dropping rows is a total nobody can audit six weeks later. `registerDataset()` in `canvas/index.html`'s own SQL engine module was patched to read the rejects table through this model before dropping the temporary tables.

**llama.cpp sidecar packaging** (`js/ai/llama-sidecar-packaging.js`, `window.DataGlowLlamaSidecarPackaging`; fetch script `scripts/fetch-llama-sidecar.mjs`). Bundle 12 established that the desktop status reports `sidecar_missing` and that `tauri.bundle.externalBin` is empty on purpose. This is the packaging itself: `sidecarFileName` produces the exact per-target-triple name Tauri resolves (`binaries/llama-server-<triple>`, `.exe` only on Windows), and `checkPackagingAgreement` fails CI on both directions of the lie -- a config that names a binary with none vendored (the build fails loudly), or a vendored binary with nothing named in the config (the app would report ready from a file the shipped bundle never included, quietly). No binary and no model weights are in this repository: `scripts/fetch-llama-sidecar.mjs` downloads on a developer's machine into `src-tauri/binaries/`, which `.gitignore` excludes, and nothing it produces is committed.

### Archetype drills expansion; R Air-Gap prebundle manifest

Bundle 18 adds three more original Drill Floor drills to the registry Bundle 16 started, and a manifest that states honestly which R packages are actually prebundled for Air-Gap sessions versus merely on a wish list.

**Archetype drills: SCD/streaks/baskets** (`js/drill-floor/drill-floor.js`, `js/drill-floor/drill-floor-data.js`, flag `archetypeDrillsExpand`). Three drills join the existing `DRILLS` registry: `scd-as-of` (a Slowly Changing Dimension Type 2 as-of price lookup join between `drill_sales` and `drill_price_history`), `streak-islands` (the longest consecutive-day activity streak per user, a gaps-and-islands problem over `drill_activity_days`), and `basket-pairs` (the most common co-occurring SKU pair across orders, over `drill_basket_lines`). Each ships deterministic seeded synthetic data, SQL/Python/R starters, an `excelNote` naming the Excel-outcome equivalent honestly, and a `goldenAnswers` block scored by the existing `scoreDrillAnswer` (`rowCount`, cross-language) plus the new `scoreDrillExtras` (SQL-only extra scalars: `totalRevenue`/`productCount`, `maxStreak`/`userId`/`islandCount`, `pairLeft`/`pairRight`/`orderCount`). Flag off, the three new drills are not registered in `DRILLS` and do not appear in the Drill Floor tab or overlay panel picker; the four Bundle 16 drills are unaffected.

**R Air-Gap prebundle manifest** (`js/polyglot/r-air-gap-prebundle.js`, `window.DataGlowRAirGapPrebundle`, flag `rAirGapPrebundle`). `r-deepen.js` (Bundle 17) fixed Air-Gap mode not blocking the two-package WebR startup fetch (jsonlite, ggplot2); this module is the manifest of INTENT versus REALITY for a target list of six R packages this product would like to ship prebundled for Air-Gap R sessions: dplyr, tidyr, ggplot2, jsonlite, readr, broom. Each entry in `R_AIRGAP_PREBUNDLE` declares an honest `availability` of `bundled`, `network-only` or `unavailable` — as of this bundle every entry is `network-only` or `unavailable`, none are `bundled`, because no local/static R package asset is checked into the repository yet. `prebundleStatusCopy` never returns an "Available offline" claim for anything except a `bundled` entry, and `resolvePackageLoad` prefers a local asset the instant one becomes bundled, with no call-site change, otherwise deferring entirely to the caller-supplied network decision (`r-deepen.js`'s `packageInstallDecision`), so this module duplicates no Air-Gap logic of its own. Pure and cross-platform: no DOM, no Node-only API, no Tauri-only API, imported by the same path from the browser bundle, a Tauri desktop shell, and a mobile PWA build. Flag off, the manifest and loader are not imported or rendered anywhere, and R Air-Gap behavior is exactly what `r-deepen.js` already does.

`test/bundle18-archetype-drills-r-airgap.test.mjs` covers both features: the three new drills' golden scalars independently re-derived from the same generator functions and matched, `scoreDrillAnswer`/`scoreDrillExtras`/`scalarMatches` pass/fail/never-throws behavior, the R Air-Gap prebundle manifest's honest-availability invariant and loader precedence, the `archetypeDrillsExpand`/`rAirGapPrebundle` flags present and ON with dependency flags untouched, and canvas integrity/inject parity for both additions.

### DuckDB load harden; Repair Ledger on the RECEIPT spine; thin replay; SQL Dojo crash-safe

Bundle 15 makes the SQL-primary path demo-reliable end to end: the engine loads even when a CDN is down, the Repair Ledger is findable from the same rail that already names a person's place in the path, a rerunnable step can actually be rerun with one confirm, and the Window Function Dojo, the SQL Dojo, cannot crash a session by opening onto a DOM it does not recognize.

**DuckDB-WASM multi-CDN load harden** (`js/sql/duckdb-load-harden.js`, flag `duckdbLoadHarden`). The SQL Editor's engine (`js/sql/sql-engine.js`) and the canvas-inlined SQL engine loader each hand-built the same jsDelivr-then-unpkg fallback with their own copy of the version string, and two copies of a pin drift the first time one is bumped and the other is not -- a demo that works from one code path and hangs from the other. `js/sql/duckdb-load-harden.js` is the one place the pinned version (`DUCKDB_WASM_PIN`) and the candidate host order live: `buildCandidateList()` returns the ordered jsDelivr, unpkg, esm.sh list both callers walk, and `summarizeAttempts()` turns a list of per-host attempts into the loading/ok/error banner state, naming the host that succeeded or every host that failed rather than going silent. It is pure data plus pure helpers with no fetch of its own, so it is testable under plain Node with no network and no DOM; the caller does the actual dynamic import in a browser. On a load failure, `ensureInit()` and the canvas loader now walk the full candidate list instead of hard-failing after the primary host, and Retry re-tries the full list rather than repeating a host already known dead. `MAX_ATTEMPTS` bounds one load pass to the length of the candidate list, so retrying means starting a new pass on purpose and not looping forever inside one. The existing `_dgShowSqlEngineBanner` surfaces which host loaded or failed; flag off, both surfaces fall back to a single hardcoded jsDelivr URL with no fallback host, matching pre-Bundle-15 behavior.

**Repair Ledger chip on the RECEIPT spine** (`js/spine/data-glow-receipt-spine-canvas.js`, `js/spine/data-glow-repair-ledger-canvas.js`, flag `repairLedgerSpine`). The Repair Ledger was reachable only from a floating button, a 25th thing to notice on a page that already has two dozen. This mounts a Repair Ledger chip directly on the RECEIPT spine rail -- the top row when the rail is open, a floating fallback chip when the rail is collapsed -- so the ledger is findable from the same strip that already names Drop, Ask, Prove, Ship, Compound. The pre-existing floating button (flag `repairLedger`) is unaffected when `repairLedgerSpine` is off, and the spine renders exactly as it did before Bundle 15 with no chip in either state when the flag is off.

**Thin Replay** (`js/spine/data-glow-repair-ledger-canvas.js`, flag `replayReceiptThin`). Every `canRerun` SQL step already carried a `rerunPlan()` -- the exact SQL that ran -- but the only thing a person could do with it was copy it out. Each such step in the ledger panel now gets a Run again button that inserts that exact SQL into the real editor and clicks the real Run button, and only after an explicit `window.confirm()` naming the SQL about to run; declining changes nothing. A Replay all SQL steps in order control appears once more than one rerunnable step exists, queuing every step behind one confirm, running sequentially, and stopping early with a toast if the SQL engine becomes unavailable mid-sequence rather than failing silently partway through. Flag off, the panel shows only the pre-existing Copy code and Re-run plan (copy-to-clipboard) actions; no button in this product runs SQL on its own without that explicit human click on Run, on or off.

**SQL Dojo crash-safe open** (canvas-only, no separate `js/` source module; flag `sqlDojoSafe`). The Window Function Dojo's DOM lookups were unguarded, so a missing panel or grid, or `populateDropdowns()` running against a missing or malformed active dataset, could throw and take a session down rather than degrade. Every lookup is now guarded end to end: a missing panel or grid no-ops with a toast instead of throwing, `populateDropdowns()` defers off the opening click's frame and guards against a missing or malformed dataset, the sidebar proxy button has a double-init guard, and the whole Dojo IIFE is wrapped in a top-level try/catch. If the SQL engine is not mounted, the Dojo still renders and still generates SQL, but Run in SQL Editor is disabled with a plain-language reason -- that disable-when-missing behavior is gated behind `sqlDojoSafe`, while the crash guards themselves stay on unconditionally, since disabling crash-safety on purpose has no product benefit. The SQL Dojo is the Window Function Dojo: one surface, two names, so a reference to either in this codebase or its docs means the same panel.

`test/bundle15-duckdb-harden-ledger-spine-replay-dojo.test.mjs` (39 cases) covers candidate-list shape/order/immutability, `rewriteBundleUrl` and `summarizeAttempts` behavior, the retry-ceiling logic, the ledger's `canRerun`/`rerunPlan` gating reused from `js/spine/repair-ledger.js`, and a no-U+2014 sweep across every file this bundle touches. `test/bundle15-canvas-ui.test.mjs` (22 assertions, real headless Chrome) covers the spine ledger chip mount and flag-off state, the replay confirm-gate decline-versus-accept paths, `replayReceiptThin` flag-off removing only the Run again control, the Dojo's safe open against a bare DOM stub with zero uncaught page errors, and the Dojo's Run-disabled state with a plain message when no SQL engine is present.

### Repair Ledger; PQ-parity recipes; Arrow bridge deepen; llama sidecar fetch status; polyglot project lanes

Bundle 14 adds the Applied Steps list DataGlow never had, a full DuckDB-SQL answer to the fifteen most common Power Query step shapes, a fourth Arrow-bridge state between the existing ready/partial/missing status and plain JSON, an honest three-state read on whether a fetched llama.cpp sidecar is actually wired into the desktop build, and four short cards naming what each runtime here can and cannot carry end to end.

**Repair Ledger** (`js/spine/repair-ledger.js`, pure engine, `window.DataGlowRepairLedger`; canvas `js/spine/data-glow-repair-ledger-canvas.js`, `window.DataGlowRepairLedgerUI`; flag `repairLedger`). A plain, in-memory, append-only array of steps, each with an id, timestamp, kind, engine (sql/python/r/excel/system), the code or recipe id, an input/output table hint, a receipt-friendly summary, and a status of applied, skipped, failed, or proposed. `exportLedgerJson`/`exportLedgerMarkdown` write the session out; a step whose kind is a pure SQL insert can offer `rerunPlan`, which hands back the exact SQL that ran and stops there, since whether it runs again is a Copy click like every other snippet in this product, never something the panel does on its own initiative. `wiringReport()` compares which of nine known surfaces (load, csv_quarantine, type_guard, excel_hell, sql_recipe, python_recipe, r_recipe, summarize_tiles, export) appended at least one step this session against the full known list and names the gap in plain text rather than a ledger that quietly reflects only part of what happened. Best-effort wiring in this bundle appends from the Excel type guard, SUMMARIZE profiling tiles, and PQ-parity/SQL recipe copies in `js/polyglot/data-glow-power-packs-canvas.js`; CSV quarantine and Excel Hell apply are not yet wired, which `wiringReport()` reports rather than hides.

**PQ-parity recipes** (`js/polyglot/pq-parity-recipes.js`, `window.DataGlowPqParityRecipes`, flag `pqParityRecipes`). Fifteen DuckDB SQL templates matching the Power Query step shapes people actually reach for: promote headers, change type, fill down/up, split column, three merge-queries joins (left/inner/full), append queries (`UNION ALL BY NAME`, not positional), group-by aggregate, unpivot, pivot, remove duplicates, replace/trim/clean, and a fuzzy-join sketch that names its own `O(n*m)` cost rather than hiding it behind a friendly button. `buildPqParityPack` carries the same honesty note as `js/polyglot/power-query-note.js`: this is SQL that does the same job, not Power Query M embedded in the browser, and it points at the Repair Ledger for the Applied Steps equivalent.

**Arrow bridge deepen** (`js/polyglot/arrow-bridge.js`, `window.DataGlowArrowBridge`, flag `arrowBridgeDeepen`; the underlying flag `arrowBridge` from Bundle 13 is unchanged). `buildArrowBridgeStatusV2` adds a fourth transfer-kind reading past the existing ready/partial/missing status: `arrow_ipc` (the v1 status is actually ready), `batch_bridge` (Python is ready and typed-array batches are available even without pyarrow), `json_bridge` (Python is ready but typed arrays are not), or `missing`. `encodeColumnBatch`/`decodeColumnBatch` move a typed-array-backed column across the bridge without the JSON stringify/parse round trip, and `roundTripFixture()` proves the pair against a fixed input so a regression here fails a test rather than a demo. `BATCH_BRIDGE_CEILING` restates `NEVER_UNLIMITED`: a batch bridge removes a parse cost, not the fact that a frame has to fit in memory.

**llama sidecar fetch status** (`js/ai/llama-sidecar-packaging.js` addition `fetchSidecarStatus`, flag `llamaSidecarFetch`; fetch script `scripts/fetch-llama-sidecar.mjs` with new `--dry-run`/`--status` flags; `docs/desktop-llama-sidecar.md`). Bundle 13 shipped the packaging agreement check; this closes the gap between "a binary is vendored" and "the desktop build actually ships it": `fetchSidecarStatus` reports `missing` (nothing on disk for the triple), `fetched_unwired` (vendored, but `tauri.bundle.externalBin` does not declare it, or the two disagree), or `ready`, and never reports `ready` purely from `sidecarPresence()` without also checking the packaging agreement. `--dry-run` on the fetch script proves out the whole flow, including a failed placement, with zero filesystem writes, and `--status` reports the same three states from the command line for a developer setting up llama.cpp locally.
...
### Proof Harness v1: Second Engine Rule corroboration, Regression Vault, Proof Cartridge, review Inbox, AMBER staleness

The second slice of the MASTER PROMPT doctrine: "AI proposes, engines prove, human confirms. Draft elsewhere. Prove here." Composes v0 rather than replacing it. Flag `proofHarnessV1` (umbrella on top of the existing `proofHarness`). `window.DataGlowProofHarness.version` is now `2`; every v0 method (`runProofCycle`, `confirmProposal`, `createTypedProposal`, `decideVerdict`, `compareClaimToRun`, `getReceipts`, `verifyReceipts`, `resetReceipts`) is still present and behaviorally unchanged for a caller that never passes the new optional arguments.

**Second Engine Rule corroboration** (`js/proof-harness/second-engine.js`). `corroborateRun()` takes the same proposal plus a second, independently resolved run (a different engine binding, never a re-run of the identical call) and compares scalars with the same epsilon-and-trim discipline `score-claim.js` already established, returning an `agrees` boolean plus a `divergence_class` when the two disagree. `runProofCycle()` in `js/proof-harness/index.js` now threads an optional `runSecondEngine`/`secondEngineName` pair through, and `verdict.js`'s `gateGreen()` blocks a `GREEN` verdict outright when the two engines disagree, downgrading to `RED` with reason code `corroboration-disagree` rather than ever surfacing a silently unconfirmed `GREEN`. `js/proof-harness/receipt.js`'s predicate already carried a `corroboration` field since v0; v1 is the first thing that actually populates it.

**Regression Vault** (`js/proof-harness/vault.js`). `createVault()` builds an in-memory store persisted to `localStorage` under the constant `dataglow.proofHarness.vault.v1`; every `RED` verdict from `runProofCycle()` and every `rejectProposal()` call is auto-appended as a frozen vault test (the expected shape plus the failing run captured verbatim), and `runVault()` replays every stored test against a caller-supplied engine so a claim that was once wrong and got fixed cannot quietly regress back to the same wrong number without a test catching it.

**Proof Cartridge export and import** (`js/proof-harness/cartridge.js`). `exportCartridge()` serializes a proposal, its verdict, and its receipt chain into a single `dataglow/proof-cartridge/v1` JSON document (constant `PROOF_CARTRIDGE_TYPE`) that always zeroes out `rows` (a cartridge is proof of a claim traveling, never a copy of the underlying data), hashed as `sha256:<hex>` over the canonicalized payload. `importCartridge()` and `verifyCartridgeHash()` re-derive that hash and refuse a tampered or hand-edited cartridge rather than trusting the embedded verdict blindly.

**Review Inbox** (`js/proof-harness/inbox.js`). `createInbox()` is a small state machine over the closed vocabulary `INBOX_ITEM_STATUSES`: `pending-prove`, `awaiting-confirm`, `red`, `gray`, `amber`, `confirmed`, `rejected`. A claim that has not yet been proven and a claim that was proven but not yet confirmed by a human are never rendered as the same thing; `itemFromCycleResult()` derives the right status straight from a `runProofCycle()` result so the canvas UI never hand-computes it.

**AMBER for a stale digest** (`js/proof-harness/verdict.js`, `js/proof-harness/index.js`). `VERDICT_STATES` is now `GREEN`/`RED`/`GRAY`/`AMBER`. `computeStaleness()` in `index.js` compares a proposal's live digest against the digest an earlier receipt was bound to; a mismatch downgrades what would otherwise be treated as already-proven to `AMBER` with reason code `stale-digest`, and the UI copy tells the user to press Prove again rather than trusting a receipt bound to an older statement.

**Tabbed canvas panel** (`js/proof-harness/data-glow-proof-harness-canvas.js`). Adds a tab bar (`Inbox` default, `Prove`, `Vault`, `Cartridge`) behind the `proofHarnessV1` flag. With the flag off, only the unchanged v0 Prove surface renders, byte-for-byte, so a rollback of `proofHarnessV1` alone (leaving `proofHarness` on) fully restores v0 behavior. No em dash in any visible text.

`test/proof-harness-v1.test.mjs` covers all five acceptance gates from `PROOF_HARNESS_V1_SPEC.md`: corroboration blocking `GREEN` on disagree, vault capture-on-RED and capture-on-reject plus `runVault()` replay, cartridge export/import round-tripping with a zero-row guarantee and hash tamper detection, inbox status transitions across the full vocabulary, and `AMBER` firing only on an actual digest mismatch, alongside a full re-check that every v0 method and outcome is unchanged.

### Proof Harness v2.0 foundation: adversary pack, Excel claim path, Proof Mesh attestation

The foundation slice of `PROOF_HARNESS_V2_SPEC.md`, composing v0/v1 rather than replacing them. Flag `proofHarnessV2` (umbrella on top of the existing `proofHarness`/`proofHarnessV1`).

**Adversary pack** (`js/proof-harness/adversary.js`). `buildMetamorphicRewrites()` recognizes a simple `SELECT ... FROM t [WHERE ...] [GROUP BY ...]` shape and returns at least 5 semantically-equivalent SQL rewrites (whitespace/case normalize, a redundant `WHERE 1=1`/`AND 1=1`, an outer `SELECT * FROM ( <inner> )` wrapper, a column-list reorder or a second tautology fallback, an `ORDER BY` injection, and a `NOT NOT` double-negate), honestly returning `[]` when the statement is not that shape rather than inventing a pass. `buildBoundaryProbes()` adds a cheap empty-table (`WHERE 1=0`) probe plus a null-safe `COUNT(1)` variant when `COUNT(*)` appears in the select list, max 3 probes. `runAdversaryPack()` executes every rewrite and probe through the caller's injected `runQuery`, compares each result back to `primaryRun` using the same BigInt-coerced epsilon-and-trim discipline `second-engine.js`'s `corroborateRun()` already established, and only ever reports `strengthensGreen: true` when it actually ran, zero attacks failed, and at least `ADVERSARY_MIN_REWRITES` (5) passed. A false GREEN is a release blocker (doctrine #7), so a broken rewrite counts as a failed attack in the report, never a silently dropped one, and a statement outside the recognized shape is honestly `skipped: true`, never forced into an invented pass.

**Excel claim path** (`js/proof-harness/excel-claim.js`). `parseExcelAggregateClaim()` recognizes exactly `SUM(col)`, `COUNT(col)` (including `COUNT(*)`), `AVERAGE(col)`/`AVG(col)`, with an optional trailing `on`/`from <table>` clause, and REJECTS with a specific reason (never a guess) any multi-sheet 3D reference, cell range, VBA/macro text, nested function, or unrecognized aggregate name. `excelClaimToSql()` maps a parsed claim to a quoted DuckDB `SELECT` statement (the same `quoteIdent` identifier-quoting discipline `js/polyglot/sql-deepen.js` already established) so an Excel-style claim enters the SAME prove cycle as a hand-written SQL claim, never a bespoke Excel-only verdict path; `excelClaimTextToSql()` is the one-shot parse-then-map convenience. Deliberately narrow, per `PROOF_HARNESS_V2_SPEC.md` pillar B3: no VBA, no multi-sheet 3D references, no full workbook transpile, no writing back to `.xlsx`.

**Proof Mesh attestation** (`js/proof-harness/mesh-attestation.js`). `exportMeshAttestation()` builds a row-free `dataglow/proof-mesh-attestation/v1` document (proposal digest, statement text, `expected`, verdict state/reasonCode, receipt hash, environment, `schemaFingerprint`, `inputDigest`, and a content `attestationHash`), refusing outright (`{rejected:true, reason}`) if any forbidden row-bearing key (`rows`, `samples`, `csv`, `cells`, `sheetData`) is found anywhere in the input tree, at any depth. `importMeshAttestation()` and `verifyMeshAttestationHash()` re-parse and re-hash an attestation to catch a tampered or hand-edited document before it is trusted or compared. `compareMeshAttestations()` lets two attestations agree purely on `proposalDigest` equality (or matching `statement` + `expected`) plus matching `verdict.state`, with `schemaFingerprint`/`inputDigest` divergence surfaced additionally but never gating agreement on its own -- two devices can agree or diverge on a proof entirely offline, with zero source rows ever crossing the wire. This is the foundation slice per `PROOF_HARNESS_V2_SPEC.md` pillar C: live WebRTC exchange over Rooms' mesh transport is residual, not built here.

**Canvas wiring** (`js/proof-harness/data-glow-proof-harness-canvas.js`). Adds an Excel-fill control on the Prove tab (backed by `excel-claim.js`'s pure engine), an adversary pass/fail note, and mesh attestation export/compare controls, all behind the new `proofHarnessV2` flag. With the flag off, the v1 surface (`Inbox`/`Prove`/`Vault`/`Cartridge`) renders byte-for-byte unchanged -- so a rollback of `proofHarnessV2` alone fully restores v1 behavior, and all three v2.0 pure modules stay independently importable and evaluable even with `proofHarnessV2` off. No em dash in any visible text; no merge or publish performed by this change.

`test/proof-harness-v2-adversary-excel-mesh.test.mjs` covers `buildMetamorphicRewrites()`'s minimum-5-rewrite floor and honest `[]` skip on non-simple-SELECT shapes, `buildBoundaryProbes()`'s empty-table and null-safe-count probes, `runAdversaryPack()`'s pass/fail/skip paths (including a rejecting `runQuery` counted as a failed attack, never dropped), `parseExcelAggregateClaim()`'s accept/reject matrix (`SUM`/`COUNT`/`AVERAGE`/`AVG`, `on`/`from` table clause, 3D refs, ranges, VBA, nested functions), `excelClaimToSql()`/`excelClaimTextToSql()`'s quoted-SQL output, and `exportMeshAttestation()`/`importMeshAttestation()`/`verifyMeshAttestationHash()`/`compareMeshAttestations()`'s row-free guarantee, forbidden-key rejection at any depth, tamper detection, and agree/diverge comparison logic.

### Question Scout (A49): local keeper proposals, deterministic filter + template fallback, Send to Prove

Implements `A49_QUESTION_SCOUT_SPEC.md`'s doctrine: AI proposes, human chooses, engines prove, human confirms. Flag `questionScout` (default on).

**Pure engine** (`js/question-scout/question-scout.js`, `window.DataGlowQuestionScout`). `buildProfileStrip(tables)` builds a deterministic schema-and-sample-stats profile of the loaded table(s) -- never raw rows to any model. `scoreCandidate()`/`rankCandidates()` are the SPEC's four-hit deterministic keeper filter: business actor/ops language, a real column/table reference, a checkable metric type (`count`/`rate`/`share`/`delta`/`sum`/`avg`), and SELECT-only SQL, with penalties for viz vanity and columns not present in the profile. `templateCandidatesFromProfile()` is the five-template model-free fallback (grain check, null rate on the highest-null column, top-N frequency on the top categorical column, min/max/avg on the first numeric column, distinct count on an id-like column), every candidate labeled `template (no model)` so a user always knows whether a proposal came from a model or a template. `buildScoutPrompt()`/`parseModelCandidates()` do prompt construction and tolerant JSON parsing for the on-device model path -- pure string logic only, this module never calls a model itself. `buildProvePrefill()` maps a kept candidate to the Proof Harness's `claimText`/`statement` fields without ever executing anything. `buildBrowseGrounding()`/`annotateUnverifiedNumbers()` keep the profile-grounded Browse-mode chat honest, flagging any numeric assertion `unverified — run Prove`. `addKeeper()`/`removeKeeper()` enforce the max-5 keepers tray.

**Canvas panel** (`js/question-scout/data-glow-question-scout-canvas.js`). A slide-in panel with the honest cheating-boundary banner, the profile strip, a Propose button that tries the already-inlined local AI bridge (`window.OnDeviceLLM` from `js/narrative/ondevice-llm.js`, reusing its `loadModel()`/`isModelLoaded()` -- no second model path, no forced download) and falls back to deterministic templates immediately when the model is cold/absent/unavailable, a ranked candidate list with Keep/Edit/Reject, the keepers tray, a Send to Prove action per keeper that prefills the Proof Harness's `#dg-ph-claim`/`#dg-ph-statement` fields (or dispatches a `dataglow:proof-harness-prefill` `CustomEvent` as a fallback) without ever running SQL itself, and a profile-grounded Browse mode chat. Keep/Edit/Reject only ever change in-memory panel state; this panel never auto-mutates data and never marks anything proven on its own. With `questionScout` off, the button/panel are never mounted in `canvas/index.html` (the pure engine may still be evaluated and unit-tested since it makes no DOM change itself); no other engine, flag, or panel is touched by this flag.

`test/a49-question-scout.test.mjs` covers `buildProfileStrip()`, the four-hit keeper filter's score/rank behavior, all five fallback templates, prompt construction and tolerant JSON parsing, `buildProvePrefill()`, Browse-mode grounding and unverified-number annotation, and the max-5 keepers tray (89 assertions) -- wired into CI as the `a49-question-scout` job in `.github/workflows/job-ci-batch-03.yml`. Registered as the `question-scout` capability in `capability-map.manifest.json` + `docs/capability-map.md` (`node test/capability-drift.test.mjs` clean).
