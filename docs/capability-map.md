...
The canvas surface is one calm panel, `js/transforms/data-glow-time-joins-canvas.js` (`window.DataGlowTimeJoinsUI`, panel `#time-joins-view`), reachable from a Time and joins button. It computes nothing: every number and every sentence comes from an engine. Finding first, notes naming what could not be done, a preview, then the SQL underneath. Apply takes two clicks and the second one names the consequence in rows, because three of these four change the row count and one can multiply it. Undo restores the pre-image. Each tab is behind its own flag and an off tab is not rendered, so a disabled capability leaves no dead control; with all four off, neither the panel nor the button mounts. Web, desktop, PWA.
