...
| The Bench: dataset lineage tracking + tab wiring (Batch 2 of 3) | `js/app-shell/bench-shell.js` (engine, Batch 1), `js/app-shell/main.js` (`logBenchStep`/`renderBenchStoryStrip`/`resetBenchLineage`, Batch 2) | BEHIND FLAG (`theBench`) | MED | Pure lineage algebra (`createLineage`/`addStep`) + a thin story-strip renderer (`renderStoryStrip`) from Batch 1, now wired live: an `upload` step logs on every real dataset load (`resetPanelStates`), a `sql`/`python`/`r` step logs on every successful run in each tab, and a `clean` step logs on every applied cleaning fix. The SAME lineage renders identically into `#bench-story-sql` / `#bench-story-python` / `#bench-story-r` — proven by `test/e2e-bench-story-strip.test.mjs` switching tabs and asserting byte-identical strip text, not three separate per-tab histories. A fresh dataset load resets the story rather than carrying over a prior dataset's lineage. Batch 3 adds the Validate & Trust slide-over; a real shared-grid rendering engine (replacing three separate result panes) remains a separate, larger, not-yet-scoped lift. |
...
| Trust Passport (Batch 1: engine) | `js/provenance/trust-passport.js`<br>`test/trust-passport.test.mjs` | BEHIND-FLAG | HIGH | Pure composition engine answering "can I trust this, right now, in one look" without visiting four separate screens. `buildTrustPassport()` composes the EXISTING outputs of the readiness gate, AI Touch Ledger, Ownership Ledger and Agent Action Firewall into one object: readiness score, touch/ownership summary, and a fixed five-capability agent-permission table (read-aggregate: allow, read-row-level: deny, propose-repair: gate, auto-apply-repair: deny, export-outside-device: gate). Invents no new ledger, crypto, or storage; never throws on missing/malformed input. Wired into a real UI panel by Batch 1.5 (below); flag `trustPassport` still defaults off. |
| Trust Passport Export & Independent Verify (Batch 2) | `js/provenance/trust-passport-export.js`<br>`test/trust-passport-export.test.mjs` | BEHIND-FLAG | HIGH | Makes a Trust Passport portable and independently verifiable. Reuses the exact Merkle/SHA-256 primitives already shipped in `verifiable-check-seal.js` (`sealCheckResult`/`verifySeal`) rather than inventing new cryptography. `sealTrustPassport()` seals every passport field (readiness, touch/ownership summary, full permission table) into a Merkle-tree commitment; `verifyTrustPassportExport()` recomputes hashes offline, with no DataGlow install and no shared secret, and detects any edit to any field after export. `exportTrustPassport()` renders json/markdown/text. Tamper-evidence only, not a signature or certification; disclaimer shipped on every export. Its seal/verify/export controls render inside the Batch 1.5 panel only when `trustPassportExport` is also on. Flag `trustPassportExport` (default off). |
| Trust Passport Panel (Batch 1.5: UI mount) | `js/provenance/trust-passport-panel.js`<br>`test/trust-passport-panel.test.mjs` | BEHIND-FLAG | HIGH | Mounts Batch 1 + Batch 2 into a real Trust Passport tab: `buildTrustPassportPanelPlan()` is a pure, dataset-loaded readiness check; `renderTrustPassportPanel()` is the DOM presenter (summary card, permissions table, optional seal/export/verify card). Wired into `js/app-shell/main.js` (`TAB_META`, `getVisibleTabIds()`, `switchTab` dispatch, `renderTrustPassportTab()`), `js/app-shell/state.js` (`tabOrder`), `js/app-shell/command-deck-nav.js` (Trust stage), and a static panel section in root `index.html`. Gated behind the same `trustPassport` flag as the engine (still default off); export controls inside the panel additionally require `trustPassportExport`. |
