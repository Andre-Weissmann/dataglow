...
- **Command Deck adaptive next-step rail (Part 3)** — `js/app-shell/next-step-rail.js` (pure logic, no DOM): `NEXT_STEP_RULES` is a fixed-order, rules-based (no AI required) rulebook of 8 rules covering the app's real Frame→Work→Trust→Tell workflow -- load a dataset, run preflight, resolve cleaning issues (only if any were actually found), run a query, validate it, visualize it, tell the story, or (once every stage is complete for the loaded dataset) a terminal "all done" rule with no tab to jump to. `computeNextSteps({progress, tabMeta, limit=2})` walks the rules in order, returns the first `limit` whose `when(progress)` is true, and silently skips any rule whose `tabId` doesn't resolve in the caller's real `tabMeta` rather than guessing -- same drift-proofing posture as Parts 1 and 2, just applied to a suggestion list instead of a nav list. `emptyProgress()` returns the all-false default snapshot. Progress itself is tracked as a small additive `state.progress` object in `js/app-shell/state.js` (`preflightRun`, `cleanIssuesFound`, `cleanResolved`, `queryRun`, `validationRun`, `chartBuilt`, `storyBuilt` -- `hasDataset` is derived live from `getActiveDataset()`, not stored), set at the real completion point of each existing action already in `main.js` (end of `runPreflight()`, both branches of `scanClean()`'s issue scan, the per-issue fix-applied click handler, the success path of `runSqlQuery()`, the end of `runValidation()`, the success path of the Visualize tab's chart-generate handler, and the Story tab's generate handler right after `state.lastStory` is set) -- these are pure additive reads/writes of existing control flow, never new gating logic, and setting a flag never changes what the action itself does. Wired into `js/app-shell/main.js` as `renderNextStepRail()`, rendering into the new `#next-step-rail` host in `index.html` (sits at the top of the workspace content column, above every panel, styled in `css/app.css`) and called after every progress write plus from `switchTab()` and `renderSidebar()` so the rail's suggestions stay live without needing a page reload; clicking a suggestion calls `switchTab()`. Ships dark behind the `dataglowNextStepRail` flag (default OFF) -- with the flag off, `renderNextStepRail()` hides the host and renders nothing, and no progress-tracking write changes any existing behavior on its own. Independent of Parts 1 and 2 (any of the three flags can be on/off without affecting the others). A dedicated test (`test/next-step-rail.test.mjs`) regex-extracts the real `TAB_META` straight out of `main.js` at test time, same drift-proofing discipline as Parts 1 and 2, plus dedicated coverage for rule ordering, the `limit` parameter, the unknown-tab skip guard, and the terminal all-done state.
