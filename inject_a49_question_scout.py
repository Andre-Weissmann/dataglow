#!/usr/bin/env python3
"""A49 Question Scout: inject two new tracked modules into canvas/index.html.

  js/question-scout/question-scout.js
      Pure engine (profile strip, deterministic keeper filter, template
      fallback, prompt/parse helpers, Prove prefill, Browse grounding).
      Exports `export const DataGlowQuestionScout = { ... }` -- wrapped in an
      IIFE and published as `window.DataGlowQuestionScout`, same convention
      `build_engine_iife()` uses in inject_bundle18.py for a brand-new engine
      module (this script's version is a trimmed, single-purpose copy of
      that same approach, since only one engine file needs it here).

  js/question-scout/data-glow-question-scout-canvas.js
      Canvas UI (panel, button, Keep/Edit/Reject, keepers tray, Send to
      Prove, Browse mode). Already a self-contained IIFE (matches the Proof
      Harness canvas module's own convention) -- inlined verbatim, no
      rewriting needed.

Both blocks are inserted directly after the Proof Harness canvas module's end
marker (`/* ---- end js/proof-harness/data-glow-proof-harness-canvas.js ---- */`),
so Question Scout sits right next to the module it hands off to (Send to
Prove) in canvas/index.html's module order. This is a brand-new placement
(no prior canvas splice existed for either file), matching how
inject_bundle18.py's `insert_before()` placed r-air-gap-prebundle.js.

Idempotent: re-running this script re-syncs both blocks in place rather than
duplicating them.

Run with:
    python3 inject_a49_question_scout.py
Then verify with:
...
ENGINE_PATH = 'js/question-scout/question-scout.js'
CANVAS_UI_PATH = 'js/question-scout/data-glow-question-scout-canvas.js'

ANCHOR_END_MARKER = '/* ---- end js/proof-harness/data-glow-proof-harness-canvas.js ---- */'
...
def build_canvas_ui_block(path):
    ...
    like js/proof-harness/data-glow-proof-harness-canvas.js was."""
    src = read(path)
...
def guard(block, path):
    # Note: unlike inject_bundle18.py's own newly-authored engine file, the
    # SPEC's locked banner copy for Question Scout legitimately contains an
    # em dash ("Scout proposes questions... \u2014 same as a senior..."), and
    # the wider canvas already carries thousands of em dashes in injected
    # prose elsewhere, so this guard only checks for the two things that are
    # ACTUALLY unsafe to inline: a literal </script> that would terminate the
    # surrounding <script> tag early, and raw control characters.
    if '</script>' in block:
...
def main():
    ...
    engine_block = build_engine_iife(ENGINE_PATH, 'DataGlowQuestionScout')
    ...
    # Insert the UI block right after the engine block so Question Scout's
    # two files sit adjacent to each other and to Proof Harness.
