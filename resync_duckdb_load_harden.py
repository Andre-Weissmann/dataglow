#!/usr/bin/env python3
"""Bundle 18 hotfix 2: re-sync js/sql/duckdb-load-harden.js into canvas/index.html
after making SELF_HOST_BASE_URL root-absolute and adding resolveSelfHostBaseUrl().
Reuses the exact IIFE-wrap transform inject_bundle15.py used to inline this module
originally, so the tracked span in canvas stays byte-for-byte consistent with the
source module (see scripts/check-canvas-integrity.mjs).
"""
import re
import sys
import hashlib
...
PATH = 'js/sql/duckdb-load-harden.js'
...
def main():
    ...
    if spliced is None:
        sys.exit('%s: from/end marker pair not found in canvas -- expected an existing tracked span' % PATH)
    data = spliced
