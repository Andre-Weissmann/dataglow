...
async function main() {
  ...
  ok(receipt.layers.length === 23, `receipt: summarizes all 23 validation layers (got ${receipt.layers.length})`);
  ok(receipt.summary.total === 23 && (receipt.summary.pass + receipt.summary.fail + receipt.summary.warn + receipt.summary.idle) === 23, 'receipt: pass/fail/warn/idle tally accounts for every layer');
  ...
  ok(layersSection.layers.length === 22, `peer-review: layers section covers the 22 runAllLayers layers (got ${layersSection.layers.length})`);
