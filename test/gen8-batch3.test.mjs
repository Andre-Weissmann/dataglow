...
async function main() {
  ...
  ok(receipt.layers.length === 18, `receipt: summarizes all 18 validation layers (got ${receipt.layers.length})`);
  ok(receipt.summary.total === 18 && (receipt.summary.pass + receipt.summary.fail + receipt.summary.warn + receipt.summary.idle) === 18, 'receipt: pass/fail/warn/idle tally accounts for every layer');
  ...
  ok(layersSection.layers.length === 17, `peer-review: layers section covers the 17 runAllLayers layers (got ${layersSection.layers.length})`);
