// ============================================================
// DATAGLOW — Standalone Selective-Disclosure Provenance Proof Verifier
// ============================================================
// Independently re-verifies a DATAGLOW selective-disclosure proof artifact
...
//
// The only DATAGLOW code it uses is js/selective-disclosure-proof.js (+ the
// sha256Hex helper it imports from js/provenance.js), whose crypto is the
// built-in Web Crypto API (crypto.subtle) — no third-party libraries, so an
// auditor can read the code it depends on and trust the math. It NEVER touches
// the dataset — that is the entire point of the scheme.
//
// USAGE:
//   node test/verify-proof.mjs path/to/dataglow-sd-proof.json
//   node test/verify-proof.mjs            # run a built-in self-test
//
...
function report(res) {
  console.log('');
  console.log(res.valid ? '✓ PROOF VERIFIED' : '✗ PROOF FAILED VERIFICATION');
...
async function selfTest() {
  ...
  const good = await verifyProof(artifact);
  ...
  const bad = await verifyProof(tampered);
