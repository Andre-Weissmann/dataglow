...
;(function () {
  ...
  function styles() {
    ...
    var css = ''
      ...
      /* Defense in depth alongside the width-based default-collapsed fix in
         mount() below: a phone-width visitor can still tap the chip to open
         the rail on purpose, and it should never grow past a bounded slice
         of the screen even then, on any viewport, short or tall. */
      + 'pointer-events:none;max-height:42vh;overflow-y:auto}'
  ...
  /* True on a viewport too small in either dimension to give the open rail
     its own uncontested slice of the screen. 2026-09-26 fix, measured
     against real device sizes rather than guessed: at typical phone
     portrait widths/heights (390x844 down to 375x667) the rail's real
     rendered content already covers ~37-38% of the screen, from wrapping to
     its mobile layout under the existing 700px width breakpoint (the same
     one .dg-sp-loaded already uses) -- so width alone catches every phone
     portrait case. But a wide-and-short viewport (740x400, a small tablet
     or phone in landscape) is >700px wide and still measured at ~42% cover
     with the rail open, so height needs its own threshold too: 500px was
     chosen because every phone-portrait height tested (568 and up) already
     clears it comfortably via the width check, so this only fires for the
     landscape/short case width doesn't catch, not tightening anything
     that already worked. matchMedia over a fixed window.innerWidth/Height
     read so it keeps re-evaluating if the viewport is resized/rotated. */
  function tightViewport() {
    try {
      if (typeof window.matchMedia === 'function') {
        return window.matchMedia('(max-width:700px), (max-height:500px)').matches;
      }
    } catch (_e) {}
    try { return window.innerWidth <= 700 || window.innerHeight <= 500; } catch (_e2) { return false; }
  }
  ...
  function mount() {
    ...
    /* On a phone-width viewport the rail was covering roughly the bottom
       third of every tab on first load, since it otherwise defaults open
       until the user has dismissed it once (see SEEN_KEY above) -- most
       first-time mobile visitors never had. Default collapsed to the chip
       there instead; nothing about Hide/reopen/dismissed-memory changes, and
       a desktop-width visitor still sees the rail open exactly as before. */
    state.open = !remembered(SEEN_KEY) && !tightViewport();
