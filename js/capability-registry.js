...
// (Tauri) — in both, this file lives under js/ and the manifest one level up.
...
// Default dynamic importer: resolve a `js/<name>.js` manifest path to a specifier
// relative to this module and import it. Isolated so tests can inject a fake.
function defaultImporter(relFile) {
  const specifier = new URL('./' + relFile.replace(/^js\//, ''), import.meta.url).href;
