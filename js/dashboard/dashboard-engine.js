...
export var DashboardEngine = (function () {
  'use strict';
  ...
  function deriveCharts(dataset) {
    ...
    if (catIdxs.length && numIdxs.length) {
      ...
      if (s.length>=2) charts.push({type:'bar',title:'Avg '+cols[mi].name+' by '+cols[ci].name,data:s,question:'Which '+cols[ci].name+' has the highest avg '+cols[mi].name+'?'});
    }
    ...
    if (catIdxs.length) {
      ...
      if (fs.length>=2&&(catIdxs.length>1||charts.length===0)) charts.push({type:'bar',title:'Count by '+cols[ci2].name,data:fs,question:'How many records per '+cols[ci2].name+'?'});
    }
    ...
    if (dateIdxs.length && numIdxs.length) {
      ...
      if (ts.length>=2) charts.push({type:'line',title:cols[mi2].name+' trend over time',data:ts,question:'How has '+cols[mi2].name+' trended over time?'});
    }
    ...
    if (numIdxs.length) {
      ...
      if (hv.length>=5) {
        ...
        if (bd.length>1) charts.push({type:'bar',title:'Distribution: '+cols[hci].name,data:bd,question:'What is the distribution of '+cols[hci].name+'?'});
      }
    }
    ...
    if (numIdxs.length>1 && catIdxs.length) {
      ...
      if (s5.length>=2) charts.push({type:'bar',title:'Avg '+cols[mi5].name+' by '+cols[ci5].name,data:s5,question:'How does '+cols[mi5].name+' compare across '+cols[ci5].name+'?'});
    }
  ...
  function renderDashboard(dataset, report, el) {
    ...
    if (kpis.length) {
      ...
      kpis.forEach(function(kpi,i){
        ...
        card.innerHTML='<div class="dash-kpi-label">'+escH(kpi.name)+'</div>'+
          '<div class="dash-kpi-value">'+escH(kpi.value)+'</div>'+
    ...
    if (charts.length) {
      ...
      charts.forEach(function(chart){
        ...
        var q=document.createElement('div'); q.className='dash-chart-q'; q.textContent=chart.question;
        var t=document.createElement('div'); t.className='dash-chart-title'; t.textContent=chart.title;
        var cw=document.createElement('div'); cw.className='dash-chart-wrap';
        var canvas=document.createElement('canvas');
        cw.appendChild(canvas); card.appendChild(q); card.appendChild(t); card.appendChild(cw); cg.appendChild(card);
