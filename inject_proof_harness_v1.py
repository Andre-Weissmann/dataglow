#!/usr/bin/env python3
"""Re-inline the Proof Harness engine + canvas UI into canvas/index.html for v1.

Same shape as inject_proof_harness.py (v0), extended to compose the four new
v1 pure modules (second-engine.js, vault.js, cartridge.js, inbox.js) into the
SAME combined IIFE, and to re-inline the updated tabbed canvas UI module.

This script REPLACES the existing v0-injected block (bounded by the
index.js ENGINE_MARK/END_MARK and the canvas UI UI_MARK/END_MARK) rather than
inserting a second copy, since canvas/index.html is authoritative and must
never carry two competing window.DataGlowProofHarness publishers. If the v0
markers are not found, it falls back to inserting fresh before the
appinstalled anchor (first-run behavior, matching v0's inject script).
"""
import re
import sys
...
def build_engine_iife():
    ...
    return (
        ...
        + '    exportCartridge: exportCartridge,\n'
        + '    importCartridge: importCartridge,\n'
...
def main():
    ...
    if engine_start != -1 and engine_end != -1 and ui_start != -1 and ui_end != -1:
        # Replace the existing v0 (or a prior v1) injected block in place, so
        # canvas/index.html never carries two competing publishers.
        ...
        print('Replaced existing Proof Harness block with v1 (%d chars).' % len(new_block))
        return
