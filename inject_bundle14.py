#!/usr/bin/env python3
"""Bundle 14: Repair Ledger, PQ-parity recipes (DuckDB SQL, never M), the
Arrow bridge deepened with a real typed-array batch path, a hardened llama
sidecar fetch status, and the four polyglot project-lane cards.

This script is idempotent: it only touches spans it owns (marked from/end
blocks). No surgical patch this bundle; everything is a new engine module, a
new canvas UI module, or an in-place re-sync of an existing tracked module
that Bundle 14 extended.

Files spliced in, in dependency order (engines before the UIs that read them):

  js/spine/repair-ledger.js                        (new, ESM -> IIFE wrap)
  js/polyglot/pq-parity-recipes.js                 (new, ESM -> IIFE wrap)
  js/polyglot/project-lanes.js                     (new, ESM -> IIFE wrap)
  js/polyglot/arrow-bridge.js                      (re-synced, deepened)
  js/ai/llama-sidecar-packaging.js                 (re-synced, deepened)
  js/spine/data-glow-repair-ledger-canvas.js       (new, already an IIFE)
