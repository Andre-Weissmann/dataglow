#!/usr/bin/env python3
"""Inline Bundle 4 (Trust Ledger + Publish-Safe) into canvas/index.html.

Same shape as inject_notebook_app.py. Three modules go in right before
window.addEventListener('appinstalled', so they boot with the other canvas
surfaces:

  js/provenance/trust-ledger.js               pure engine, ESM -> IIFE
  js/gate/publish-safe.js                     pure engine, ESM -> IIFE
  js/provenance/data-glow-trust-ledger-canvas.js  canvas UI, already an IIFE
...
surfaces read the engines off window. The canvas UI already carries its own
from/end markers and is inlined verbatim.

Bundle 4 also EDITED an already-inlined module, js/intelligence/data-glow-
notebook-app-canvas.js, to wire Publish-Safe into the Save-as-app sheet. Its
inlined span is re-synced in place here rather than by hand, because the two
copies drifting is exactly what npm run check:canvas-integrity fails on.

No CSS block: both UIs inject their own <style> at runtime, the way the
surfaces they mount beside already do.
...
ENGINES = [
    ('js/provenance/trust-ledger.js', 'DataGlowTrustLedgerEngine', 'DataGlowTrustLedger'),
    ('js/gate/publish-safe.js', 'DataGlowPublishSafeEngine', 'DataGlowPublishSafe'),
]

# Canvas UIs, inlined verbatim because they are already IIFEs with markers.
UIS = ['js/provenance/data-glow-trust-ledger-canvas.js']

# Already inlined by an earlier bundle, edited by this one, so re-sync in place.
RESYNC = ['js/intelligence/data-glow-notebook-app-canvas.js']
...
def build_engine_iife(path, window_name, ns_name):
    ...
    inside the IIFE: the source's own `try { window... }` tail would work, but
    keeping the shape identical to inject_notebook_app.py matters more than
    saving three lines, because the next person reads both.
    """
    src = read(path)
...
def main():
    ...
    print('canvas %d -> %d bytes (%+d)' % (before, len(data), len(data) - before))
