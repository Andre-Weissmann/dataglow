...
## Concept in progress: Verified Cross-Org Truth Network

**Brainstormed:** 2026-07-12. **Combines:** a generalization of Data Diplomacy's reconciliation engine
(currently hardcoded to one demo scenario) with a brand-new Cross-Table Relational Rules layer that
directly fixes Run 4's P1 finding ("No cross-table temporal/relational plausibility checking exists at
all"). Ranked highest of 6 candidates (over Glass Box recruiter mode, Federated Trust Network, AI Agent
Handoff Protocol, and two others) on value × buildability × philosophy-fit: it reuses 100% already-
shipped crypto/transport (SHA-256 provenance hashing, WebRTC mesh, Trust Beam) while closing the single
biggest validation gap found in testing. No duplication risk confirmed against `capability-map.md` —
Data Diplomacy's engine is real but demo-only; cross-table checking genuinely didn't exist before this.

**Concept:** two parties each load their own table locally (claims vs. payer ledger, in the reference
scenario). Nothing raw crosses a server — only a sealed claim + verdict are exchanged peer-to-peer. The
Cross-Table Relational Rules engine traces any discrepancy to the exact row-level cause (e.g. 16
`claim_id` rows present in one table but missing from the other, netting to an exact dollar gap) rather
than just reporting a netted total mismatch. Requires human two-key approval — the engine never auto-
merges or auto-adjusts either side's numbers.

**Live preview (step 8b):** desktop + mobile mockup deployed 2026-07-12, rendered inside DataGlow's real
CSS/font/theme shell (`/home/user/workspace/dataglow_preview/`) — verified no overflow/wrapping issues
at either viewport.

**Batching (safety assessment, 2026-07-12):** split into 3 batches rather than one shot — the rules
engine is new validation logic (higher risk, isolated) while wiring it into real Data Diplomacy data
touches existing reconciliation/demo code (bigger blast radius). UI polish is cosmetic and last.

- **Batch 1 — Cross-Table Relational Rules engine (pure logic + tests).** ✅ SHIPPED 2026-07-12.
  [PR #197](https://github.com/Andre-Weissmann/dataglow/pull/197), merged as `82d5a43`. New
  `js/validation/cross-table-rules.js`: `checkCrossTableRule({ tableA, tableB, joinKeyA, joinKeyB, rule })`
  supports `date_after` (death-date washout w/ grace window — closes the Run 4 P1 finding) and
  `set_difference` (claims-total mismatch, matches the mockup's 16-row/$8,562.25 example) rule kinds;
  never throws, returns safe idle result on bad input. `summarizeCrossTableCheck()` renders a plain-
  language verdict. 17 tests in `test/cross-table-rules.test.mjs`, all passing. New `crossTableRules`
  flag added to `flags.manifest.json`, **defaults to `false` (dark, unwired)** — zero UI files touched,
  zero existing modules touched, zero Data Diplomacy code touched. Capability map docs updated in the
  same PR per the drift-detector convention. All 34 CI jobs passed (including `tauri-smoke`) before merge.
- **Batch 2 — Wire the rules engine into real Data Diplomacy claims (not started).** Will touch
  `js/diplomacy/reconciliation-engine.js` and `diplomacy-ui.js` to replace the hardcoded demo scenario
  with real loaded-table data, calling Batch 1's engine. Bigger blast radius — its own PR, its own flag
  (`dataDiplomacyRealData`), reviewed independently of Batch 1.
- **Batch 3 — UI polish to match the mockup's visual treatment (not started).** Rule-trace box styling,
  verdict banner, two-key approval row — cosmetic only, safe, last.

**Enable is always separate from build/merge.** `crossTableRules` ships dark in Batch 1; flipping it to
`true` requires its own later `confirm_action` naming the flag explicitly, on its own one-line commit —
never bundled with any batch's build/merge step, per this repo's standing rule.
